...
. 選擇*下一步*。
. 在 * 組態 Volume * 窗格中、選取現有的磁碟區或為新的資料存放區建立新的磁碟區：
+
[role="tabbed-block"]
====
.選取現有的 Volume
--
如果您選取現有的 Volume 、則必須符合下列先決條件：

** 若要使用 Volume 存取群組：
+
... 建立啟用 512e 的新磁碟區。
... 將磁碟區新增至包含一或多個目標主機啟動器的存取群組。


** 若要使用 CHAP ：
+
... 確定已為每個目標主機 iSCSI 介面卡設定 CHAP 。
... 使用下列其中一個選項、建立啟用 512e 的新磁碟區：
+
**** 使用具有每個目標主機適當 CHAP 設定的帳戶。
**** 建立帳戶並設定目標和啟動器機密。


... 檢視 Volume 詳細資料。
... 將 Volume IQN 新增至每個目標主機 iSCSI 介面卡靜態探索表。
