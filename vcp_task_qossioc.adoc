...
NOTE: 對於 NetApp HCI 部署，預設密碼在安裝期間隨機產生。若要確定密碼，請參閱本文中的步驟 4 https://kb.netapp.com/on-prem/solidfire/Element_OS_Kbs/mNode_Status_Network_Down_or_Down_in_the_mNode_Settings_tab_of_Element_Plugin_for_vCenter_VCP["KB"^]。
...
NOTE: 如果狀態出現以下任何情況，請參閱 https://kb.netapp.com/on-prem/solidfire/Element_OS_Kbs/mNode_Status_Network_Down_or_Down_in_the_mNode_Settings_tab_of_Element_Plugin_for_vCenter_VCP["KB"^]進行疑難排解：* `Down`：QoSSIOC 未啟用。* `Not Configured`：QoSSIOC 設定尚未設定。* `Network Down`：vCenter 無法與網路上的 QoSSIOC 服務通訊。mNode 與 SIOC 服務可能仍在運作。
